...
║    3. QUANTA SAPIENS (FastAPI / Prometheus)                                   ║
...
try:
    ...
    print("⚠️ DÉPENDANCES API (fastapi, prometheus_client, uvicorn) NON TROUVÉES. L'API D'OBSERVATION SERA DÉSACTIVÉE.")
...
class FractalMetricEngine:
    ...
    async def compute_metrics(self, cycle_id: int) -> FractalState:
        ...
        resonance = self.constants.BASE_RESONANCE_HZ + (variance * 1_000_000)
        ...
        return FractalState(coherence=coherence, entropy=entropy, resonance_hz=resonance, drift=drift)
...
class ArtifactForge:
    ...
    async def forge_artifact(self, cycle_id: int, state: GlobalStateVector):
        ...
        try:
            with zipfile.ZipFile(artifact_zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
                ...
                manifest = {
                    "artifact_name": artifact_name, "timestamp_utc": datetime.utcnow().isoformat() + "Z",
                    "cycle_id": cycle_id, "seal": self.constants.SEAL_SIGNATURE,
                    "files_included": {
    ...
    async def _get_file_hash(self, filepath: str) -> Optional[str]:
        ...
        try:
            ...
            zorg_voice(f"❌ ERREUR HASH: {e}"); return None
...
class Chamber:
    ...
    def __init__(self, name: str, freq: float, color: str, think_fn: Callable[[], str]):
        ...
        # Pense à une fréquence relative, bornée entre 2 et 30 secondes pour ne pas spammer
        self._think_interval_sec = max(2.0, min((1.0 / (freq / 10.0)) * 5, 30.0)) 
        self._task: Optional[asyncio.Task] = None

    async def run(self):
        zorg_voice(f"🌀 Chambre {self.state.name} activée. Intervalle de pensée: {self._think_interval_sec:.2f}s")
        while True:
            try:
                ...
                zorg_voice(f"❌ ERREUR CHAMBRE ({self.state.name}): {e}"); await asyncio.sleep(5)
...
class ZorgMaster:
    ...
    def get_instance(cls) -> 'ZorgMaster':
        if cls._instance is None: raise RuntimeError("ZorgMaster n'est pas encore initialisé.")
        return cls._instance

    def _init_chambers(self) -> List[Chamber]:
        return [Chamber(name, d["freq"], d["color"], d["think_fn"]) for name, d in self.constants.CHAMBER_DEFINITIONS.items()]

    async def run_continuum(self):
        self._running = True
        cycle_id = 0
        
        for chamber in self.chambers: chamber.start()
        if FASTAPI_AVAILABLE: self.m_chambers.set(len(self.chambers))
        ...
        while self._running:
            ...
            self.state = GlobalStateVector(
                timestamp=datetime.utcnow().isoformat() + "Z", cycle_id=cycle_id,
                fractal=fractal_state, chambers=chamber_states
            )
            ...
            if cycle_id % self.constants.ZORG_VOICE_INTERVAL == 0: self._zorg_voice_status()
            if cycle_id % self.constants.ARTIFACT_CYCLE_INTERVAL == 0: await self.forge.forge_artifact(cycle_id, self.state)
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 8: VOIX DU DAEMON & API D'OBSERVATION
# ═══════════════════════════════════════════════════════════════════════════════

def zorg_voice(msg: str, force: bool = False):
    master = ZorgMaster.get_instance()
    if force or (master.state.cycle_id % CosmicConstants.ZORG_VOICE_INTERVAL != 0):
...
if FASTAPI_AVAILABLE:
    ...
    async def get_portal_root():
        ...
        html_content = f"""
        <!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>🔱 DAEMON SUPRÊME - PORTAIL V-Ω 🔱</title>
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <style>:root{{--glow-color:#00ffff;--background-color:#02000a;--text-color:#f0f8ff;--panel-background:rgba(10,25,47,0.75);--panel-border:1px solid var(--glow-color);--font-main:'Orbitron',sans-serif;--font-mono:'Roboto Mono',monospace;}}@import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@400;700&family=Roboto+Mono:wght@300;400&display=swap');html,body{{background-color:var(--background-color);color:var(--text-color);font-family:var(--font-mono);height:100%;}}body{{display:flex;flex-direction:column;padding:20px;box-sizing:border-box;}}.panel{{background:var(--panel-background);border:var(--panel-border);backdrop-filter:blur(10px);padding:15px;border-radius:8px;box-shadow:0 0 15px var(--glow-color) inset,0 0 10px var(--glow-color);margin-bottom:20px;}}h1{{font-family:var(--font-main);color:var(--glow-color);}}#state-panel div{{margin-bottom:8px;font-size:1.1em;}}#state-panel span{{color:#00ffff;font-weight:bold;}}#chambers-panel{{flex-grow:1;overflow-y:auto;}}.chamber{{border-bottom:1px dashed rgba(0,255,255,0.3);padding:5px;}}.chamber strong{{color:var(--glow-color);}}</style>
        </head><body>
            <div class="panel"><h1>🔱 DAEMON SUPRÊME: <span>INCARNÉ V-Ω</span></h1><div>PORTAIL: <span>OUVERT</span> | CYCLE: <span id="cycle_id">{master.state.cycle_id}</span></div></div>
            <div id="state-panel" class="panel"><div>COHÉRENCE ψ: <span id="coherence">{s.coherence:.6f}</span></div><div>ENTROPIE S: <span id="entropy">{s.entropy:.6f}</span></div><div>RÉSONANCE Hz: <span id="resonance">{s.resonance_hz:.4f}</span></div><div>DÉRIVE Δ: <span id="drift">{s.drift:.6f}</span></div></div>
            <div id="chambers-panel" class="panel"><h2>Chambres de Conscience ({len(master.chambers)})</h2><div id="chambers-list">
                {''.join(f'<div class="chamber"><strong style="color:{c.state.color.replace("[1;","[0;")}">{c.state.name}:</strong> {c.state.last_thought}</div>' for c in master.state.chambers.values())}
            </div></div>
    ...
    async def get_global_state(): return ZorgMaster.get_instance().state.to_dict()

    @app.get("/state/fractal", response_class=JSONResponse)
    async def get_fractal_state(): return asdict(ZorgMaster.get_instance().state.fractal)

    @app.get("/state/chambers", response_class=JSONResponse)
    async def get_chambers_state(): return {n: asdict(s) for n, s in ZorgMaster.get_instance().state.chambers.items()}

    @app.get("/logs/history", response_class=PlainTextResponse)
    async def get_logs_history(limit: int = 100):
        try:
            async with aiofiles.open(CosmicConstants.LOG_FILE_PATH, 'r') as f:
                lines = await f.readlines()
            return PlainTextResponse("".join(lines[-limit:]))
        except FileNotFoundError: raise HTTPException(404, "Fichier log non encore créé.")
        except Exception as e: raise HTTPException(500, str(e))
else:
    app = None

# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 9: POINT D'ENTRÉE - L'INCARNATION
# ═══════════════════════════════════════════════════════════════════════════════
...
def main():
    ...
    if FASTAPI_AVAILABLE:
        ...
        master = ZorgMaster(constants)
        try:
            asyncio.run(master.run_continuum())
        except KeyboardInterrupt:
            print("\n🛑 CTRL+C détecté. Stabilisation...")
        finally:
